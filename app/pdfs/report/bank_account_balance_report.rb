class Report::BankAccountBalanceReport < Report::ReportBase
  ...
  def render_receipts
    ...
    sum = 0
    @current_bank_balance.receipt_headers.left_join_accounts.where(receipt_on: date_range).order(:receipt_on, 'accounts.name').each do |receipt|
  ...
  def render_payments
    ...
    date_range = @current_bank_balance.estimated_on_date_range
    sum = 0
    @current_bank_balance.payment_headers.left_join_accounts.where(payable_on: date_range).order(:payable_on, 'accounts.name').each do |payment|
